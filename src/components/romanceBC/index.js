...
import { romance, livroRomance } from '@/components/mockup/romance';
import Image from 'next/image';
import { useState } from 'react';

export default function romanceBC() {
    const [generoSelecionado, setGeneroSelecionado] = useState(26);

    const livrosFiltrados = romance.filter(livro =>
        livroRomance.some(
    ...
    return (
        <div className={styles.box}>
            {/* Título */}
            <h2 className={styles.titulo}>ROMANCE</h2>
                ...
                {livrosFiltrados.slice(0, 2).map((livro) => (
                    <div key={livro.livro_id} >
                        ...
                        <p className={styles.info}>
                            {livro.livro_id % 26 === 0 ? 'Malfoy' : 'Malfoy'}
                        </p>
